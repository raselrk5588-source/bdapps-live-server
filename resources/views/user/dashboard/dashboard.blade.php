                ...
                <div class="col-md-4 col-sm-6 col-lg-4">
                    <div class="premium-widget grad-blue">
                        ...
                        <div class="stats-info">
                            <h3>{{$install_app}}/{{config('app.total_install_limit')}}</h3>
                            <p>Total Install</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                @isset($total_apps)
                    @foreach($total_apps as $key=>$value)
                        <div class="col-md-4 col-sm-6 col-lg-4">
                            <a href="{{url("user/app-content/".$value['id'])}}" style="text-decoration:none;">
                                <div class="premium-widget grad-orange">
                                    <div class="bg-circle"></div>
                                    <div class="icon-box">
                                        <i class="md md-question-answer"></i>
                                    </div>
                                    <div class="stats-info">
                                        <h3>{{$value['app_remain_sms']}}</h3>
                                        <p>{{$value['app_name']}}</p>
                                        <div style="color:rgba(255,255,255,0.7); font-size:11px; margin-top:5px; text-transform:none;">
                                            {{$value['category_name']}} | <code>{{$value['ussd_code']?$value['ussd_code']:"USSD"}}</code>
                                        </div>
                                    </div>
                                </div>
                            </a>
